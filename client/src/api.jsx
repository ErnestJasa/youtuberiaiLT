...
export async function getChannels(queryObject, pageNumber = 1) {
  let includeCategoriesString = "";
  let excludeCategoriesString = "";
  queryObject.includeCategories.forEach((cat) => {
    includeCategoriesString += "&IncludeCategories=" + cat;
  });
  queryObject.excludeCategories.forEach((cat) => {
    excludeCategoriesString += "&ExcludeCategories=" + cat;
  });
  try {
    const response = await fetch(
      `${apiURL}/api/youtubers?ChannelHandle=${queryObject.search}&SortOrder=${queryObject.sortOrder}&PageNumber=${pageNumber}${includeCategoriesString}${excludeCategoriesString}`
    );
    if (response.status >= 200 && response.status < 300) {
      const data = await response.json();
      return data;
